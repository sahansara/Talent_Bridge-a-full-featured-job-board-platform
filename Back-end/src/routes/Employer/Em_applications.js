...
router.put('/applications/:applicationId/status', async (req, res) => {
  try {
    ...
    if (!application) {
     ...
     // --- Get employerId from application ---
    const employersId = application.employerId; // Make sure this field exists in your application doc
    ...
    const employer = await companiesCollection.findOne({ _id: new ObjectId(employersId) });
    if (!employer) {
