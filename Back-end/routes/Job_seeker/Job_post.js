...
router.get('/jobs', async (req, res) => {
  try {
    ...
    // Loop through jobs to enrich with company data
    const formattedJobs = await Promise.all(jobs.map(async (job) => {
      let companyName = job.company || "Company Name";
      ...
      if (job.companyId) {
        ...
        if (company) {
          ...
          logo = company.logo || company.thumbnail || logo;
        }
      }
  ...
  try {
    ...
    if (job.companyId) {
      ...
      if (company) {
        ...
        job.companyLogo = company.logo || company.thumbnail || null;
      } else {
    ...
    const formattedJob = {
      ...
      location: job.location,
      salary: job.salary,
      logo: job.companyLogo || job.thumbnail || null,
      description: job.description,
      jobType: job.jobType,
      approvedAt: job.approvedAt
    };
  ...
  try {
    ...
    const applicationData = {
      ...
      jobDetails: {
        ...
        approvedAt: job.approvedAt,
        location: job.location,
        logo: job.thumbnail || job.companyLogo,
        salary: job.salary
      },
      appliedAt: new Date(),
      updatedAt: new Date()
