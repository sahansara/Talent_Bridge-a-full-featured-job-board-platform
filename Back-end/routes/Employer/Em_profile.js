...
router.get('/profile', async (req, res) => {
  try {
    ...
    const profileData = {
      ...
      email: user.email || '',
      companyImage: user.companyImage ? `${user.image}` : '/api/placeholder/400/400',
...
router.put('/profile', async (req, res) => {
    try {
      const userId = req.user.userId;
      const { companyName, email, comDescription, contactNumber, companyWebsite } = req.body;
      const db = req.app.locals.db;
      const Companycollection = db.collection('Companies');
...
// PUT /api/company/employer/profile/change-password - Change user password
